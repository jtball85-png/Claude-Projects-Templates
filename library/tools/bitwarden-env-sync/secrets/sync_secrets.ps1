...
try {
    ...
    Write-Host "Bitwarden secrets sync"
    Write-Host "======================="
    Write-Host ""
    ...
    $python = Find-Python
    if (-not $python) {
        Write-Host "No 'python' or 'py' found on PATH -- needed to run the pull script."
        Write-Host "Install Python (python.org, or winget install --id Python.Python.3.12), then try again."
        return
    }
