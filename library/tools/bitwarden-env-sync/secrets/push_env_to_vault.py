...
another computer working on this project can pull them down instead of
re-minting tokens.

One Bitwarden secure note (named after this project) holds the whole .env
file's text as its note body -- that note is what stays in sync across
every computer via Bitwarden's own account sync, the same way a password
...
ENV_PATH = REPO / ".env"
VAULT_ITEM_NAME = f"{REPO.name} .env"
...
def main() -> None:
    ...
    print("Secrets pushed. On another computer, run pull_env_from_vault.py.")
